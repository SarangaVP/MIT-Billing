...
def list_employees(
    ...
    return employee_service.list_employees(db, search=search, lob=lob, status=status, include_deleted=include_deleted)
...
@router.post("/{employee_id}/resign", response_model=EmployeeOut)
def resign_employee(employee_id: uuid.UUID, payload: ResignRequest, db: Session = Depends(get_db)):
    return employee_service.resign_employee(db, employee_id, payload)


@router.post("/{employee_id}/transfer", response_model=EmployeeOut)
def transfer_employee(employee_id: uuid.UUID, payload: TransferRequest, db: Session = Depends(get_db)):
    return employee_service.transfer_employee_number(db, employee_id, payload)
