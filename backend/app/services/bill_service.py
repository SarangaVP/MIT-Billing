...
def _build_summary_rows(db: Session, line_items: list[BillLineItem], as_of: date | None) -> list[BillSummaryRow]:
    ...
    for li in line_items:
        ...
        # shared/pooled lines (is_shared_line) — those aren't real
        # personal employees, so excluding them here too is consistent.
        is_intern = bool(employee and employee.cadre and employee.cadre.strip().lower() == "intern")
        is_shared_line = bool(employee and employee.is_shared_line)

        if is_disconnected_this_month or is_intern or is_shared_line:
