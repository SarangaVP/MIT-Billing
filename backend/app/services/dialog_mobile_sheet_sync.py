...
def _load_lob_codes_from_separate_sheet(wb) -> dict[str, str]:
    ...
    by header name, not position, same defensive pattern used everywhere
    else in this file. Also confirmed real: this sheet only covers a
    subset of employees (roughly 65% of a typical Master sheet in
    practice) — anyone not present here simply gets no LOB code from this
    fallback, same as they'd get none if the Master sheet lacked the
    column and had no fallback at all.
    """
